...
import { addExpense, updateExpense, EXPENSE_CATEGORIES } from "../../redux/slices/expenseSlice"
import { addTransaction, updateTransaction, PAYMENT_METHODS } from "../../redux/slices/transactionSlice"
import { updateAccountBalance, selectActiveAccounts } from "../../redux/slices/accountsSlice"
import generateId from "../../utils/generateId"

const today = new Date().toISOString().split("T")[0]

const ExpenseForm = ({ onClose, editData }) => {
  ...
  const onFormSubmit = (data) => {
    ...
    if (editData) {
      dispatch(updateAccountBalance({ accountId: editData.accountId, amount: editData.amount, type: "income" }))
      dispatch(updateAccountBalance({ accountId: data.accountId, amount, type: "expense" }))

      const updated = { ...editData, ...data, amount, accountName: account?.name }
      dispatch(updateExpense(updated))
      dispatch(updateTransaction({ ...updated, id: editData.transactionId, type: "expense" }))
    } else {
      const id = generateId()
      ...
      dispatch(addTransaction({ ...expense, id: transactionId, type: "expense" }))
      dispatch(updateAccountBalance({ accountId: data.accountId, amount, type: "expense" }))
    }
      ...
      <div className="bg-surface rounded-2xl shadow-xl w-full max-w-lg max-h-[90vh] overflow-y-auto">
        ...
        <form onSubmit={handleSubmit(onFormSubmit)} className="px-6 py-5 space-y-4">
          ...
          <div className="flex gap-3 pt-1">
            ...
            <button
              type="submit"
              className="flex-1 bg-expense text-white rounded-lg py-2 text-sm font-medium hover:opacity-90 transition-opacity"
            >
